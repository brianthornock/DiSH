...
void updateSampleTime() {
  int tempSampleTime = round(tapTime / multiplier);
  ...
  // If we are using the pattern, handle everything for that here
    ...
    int currPatternMillis = millis();
    if (currPatternMillis - prevPatternMillis >= patternSampleTime) {
      updateLEDInterval = 1;
      ...
      patternCount++;
      patternSampleTime = round(tapTime / patternMultiplier);
      tempSampleTime = patternSampleTime;
      prevPatternMillis = currPatternMillis;
    }
